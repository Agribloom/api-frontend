  ...
  <div class="table-style">
    <div v-if="!hasError">
      ...
      <div v-else>
        ...
        <div class="table-controls" v-if="controls">
          ...
          <div class="v-control m-2" style="width: 70px ; margin-left: 0;">
            <div class="select">
        ...
        <nav class="pagination" v-if="paginate.show">
          <span class="page-stats hint">
            {{ paginate.from }} - {{ paginate.to }} of
            {{ paginate.length }} result(s) found
          </span>
    ...
    <div v-else>
      <alert-item :messages="errors" :title="'Oop!'">
        <div @click="refreshData" class="cbtn" v-if="tryAgain">
    ...
    methods: {
        ...
        loadDataFromApi () {
            return window.axios
                ...
                .catch(e => {
                    this.addError(e.message + ': Cannot retrieve data')
                    this.tryAgain = true
                })
        },
        /* Checks that the rows and columns data are consistent */
        assertFieldData () {
        ...
        loadData () {
            ...
            try {
                // Ensures api and data props are not left empty
                if (!this.api && !this.data.length) {
                    this.$emit('noData')
                    this.loading = false
                    ...
                    try {
                        if (this.data.length) {
                            ...
                            // if api value is not empty
                            this.url = this.api
                            // Resolve api promise
                            /* Promise.resolve(this.loadDataFromApi()).then(value => {
                                    this.rows = value !== undefined ? value : []
                                    // Check the fidelity of the rows with the column
                                    this.assertFieldData()
                                }) */
